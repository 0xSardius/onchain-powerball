...
contract OnchainPowerball is Pausable, Ownable, ReentrancyGuard {
    ...
    event PrizeAwarded(address indexed winner, uint256 amount, uint8 matchCount, bool isJackpotTier);
    ...
    function submitEntry() external payable whenNotPaused nonReentrant {
        ...
        // Generate entry hash using block variables and sender // should upgrade to VRF for production
        ...
        // Store entry
        entries[msg.sender].push(Entry({txHash: txHash, timestamp: block.timestamp, lastDigits: lastDigits}));

        // Track daily players
        uint256 drawingDay = (block.timestamp / 1 days) * 1 days;
        if (
            dailyPlayers[drawingDay].length == 0
    ...
    function conductDrawing() external whenNotPaused nonReentrant {
        ...
        for (uint256 i = 0; i < players.length; i++) {
            ...
            for (uint256 j = 0; j < playerEntries.length; j++) {
                ...
                uint8 matches = countMatches(playerEntries[j].lastDigits, winningDigits);
            ...
            if (tierWinners.length > 0) {
                if (tier.percentage > 0) {
                    ...
                    for (uint256 i = 0; i < tierWinners.length; i++) {
                        ...
                        emit PrizeAwarded(tierWinners[i], prizePerWinner, tier.requiredMatches, true);
                    }
                    jackpot -= tierPrize;
                } else if (tier.fixedPrize > 0) {
                    // Fixed prize
                    for (uint256 i = 0; i < tierWinners.length; i++) {
                        require(reserveFund >= tier.fixedPrize, "Insufficient reserve for fixed prizes");
                        payable(tierWinners[i]).transfer(tier.fixedPrize);
                        reserveFund -= tier.fixedPrize;
                        emit PrizeAwarded(tierWinners[i], tier.fixedPrize, tier.requiredMatches, false);
                    }
                }
            }
        }
    ...
    function getLastNHexDigits(bytes32 data, uint256 n) internal pure returns (string memory) {
        ...
        for (uint256 i = 0; i < n * 2; i++) {
            ...
            hexDigits[n * 2 - 1 - i] = digit < 10 ? bytes1(uint8(48 + digit)) : bytes1(uint8(87 + digit));
            lastNDigits >>= 4;
        }
    ...
    function countMatches(string memory entryDigits, string memory winningDigits) internal pure returns (uint8) {
    ...
    function canSubmitEntry(address player) public view returns (bool) {
        // Must be at least 5 minutes before next drawing
        require(block.timestamp <= nextDrawingTime - 5 minutes, "Too close to drawing");

        if (entries[player].length == 0) return true;
        Entry[] memory playerEntries = entries[player];
        Entry memory lastEntry = playerEntries[playerEntries.length - 1];

        // Check if last entry was in current drawing period
    ...
    function getDrawingWinners(uint256 drawingTime, uint8 matches) external view returns (address[] memory) {
